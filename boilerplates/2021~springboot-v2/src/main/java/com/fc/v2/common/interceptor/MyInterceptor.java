...
public class MyInterceptor  implements HandlerInterceptor {
	/*************演示模式需要 start*************/
	/**需要被拦截的post请求url**/
	public static List<TSysInterUrl> posturllist=new ArrayList<TSysInterUrl>();
	/**注入拦截器表的service**/
	public static ITSysInterUrlService interUrlService= SpringUtils.getBean(ITSysInterUrlService.class);
	/**
	 * 初始化静态块，避免多次创建调用
	 * 需要拦截的请求集合
	 */
	static {
		//post
		posturllist=interUrlService.selectTSysInterUrlList(new QueryWrapper<TSysInterUrl>());
...
//		geturllist.add("/quartz/delete");
	...
	public Boolean ifurl(HttpServletRequest request, HttpServletResponse response) {
		...
		for (TSysInterUrl sysInterUrl : posturllist) {
