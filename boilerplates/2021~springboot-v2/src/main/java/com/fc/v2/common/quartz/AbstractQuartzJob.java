...
public abstract class AbstractQuartzJob implements Job {
    ...
    {
    	TSysQuartzJob sysJob = new TSysQuartzJob();
     ...
     * @param sysJob 系统计划任务
     */
    protected void before(JobExecutionContext context, TSysQuartzJob sysJob)
    {
     ...
     * @param sysJob 系统计划任务
     */
    protected void after(JobExecutionContext context, TSysQuartzJob sysJob, Exception e)
    {
        Date startTime = threadLocal.get();
        threadLocal.remove();

        final TSysQuartzJobLog sysJobLog = new TSysQuartzJobLog();
        ...
        //  这里获取service然后插入库中
        SpringUtils.getBean(ITSysQuartzJobLogService.class).insertTSysQuartzJobLog(sysJobLog);
    }
     ...
     * @param sysJob
     */
    protected abstract void doExecute(JobExecutionContext jobExecutionContext, TSysQuartzJob sysJob) throws Exception;
}
