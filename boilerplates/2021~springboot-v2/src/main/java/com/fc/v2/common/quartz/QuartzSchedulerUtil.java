...
public class QuartzSchedulerUtil {
    ...
    public void init() throws SchedulerException {
        ...
        List<TSysQuartzJob> quartzJobs = sysQuartzJobService.selectTSysQuartzJobList(new QueryWrapper<TSysQuartzJob>());
        for (TSysQuartzJob job : quartzJobs) {
            try {
                //防止因为数据问题重复创建
...
//        }
     ...
     * @throws SchedulerException
     */
    public boolean modifyJob(TSysQuartzJob job) {
     ...
     * @return
     */
    public boolean resumeJob(TSysQuartzJob job) {
        boolean bl = false;
        try {
            //JobKey定义了job的名称和组别
     ...
     * @return
     */
    public boolean deleteJob(TSysQuartzJob job) {
        boolean bl = false;
        try {
            //JobKey定义了job的名称和组别
            JobKey jobKey = JobKey.jobKey(ScheduleConstants.TASK_CLASS_NAME + job.getId(), job.getJobGroup());
     ...
     * 获取jobKey
     */
    public JobKey getJobKey(TSysQuartzJob job) {
     ...
     * @return
     */
    public boolean pauseJob(TSysQuartzJob job) {
        boolean bl = false;
        try {
            //JobKey定义了job的名称和组别
            JobKey jobKey = JobKey.jobKey(ScheduleConstants.TASK_CLASS_NAME + job.getId(), job.getJobGroup());
     ...
     * @throws SchedulerException
     */
    public boolean checkJobExists(TSysQuartzJob job) throws SchedulerException {
     ...
     * @return
     */
    private static Class<? extends Job> getQuartzJobClass(TSysQuartzJob sysJob) {
