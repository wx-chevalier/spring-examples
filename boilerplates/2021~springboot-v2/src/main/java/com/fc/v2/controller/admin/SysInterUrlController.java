...
public class SysInterUrlController extends BaseController {
    ...
    @Autowired
    private ITSysInterUrlService sysInterUrlService;
    ...
    public ResultTable list(Tablepar tablepar, String searchText) {
        ...
        queryWrapper.like(StringUtils.isNotEmpty(searchText), "inter_name", searchText);

        PageHelper.startPage(tablepar.getPage(), tablepar.getLimit());
        PageInfo<TSysInterUrl> page = new PageInfo<TSysInterUrl>(sysInterUrlService.selectTSysInterUrlList(queryWrapper));
    ...
    @ResponseBody
    public AjaxResult add(TSysInterUrl sysInterUrl) {
        int b = sysInterUrlService.insertTSysInterUrl(sysInterUrl);
        if (b > 0) {
            return success();
        } else {
            return error();
    ...
    public AjaxResult copy(@PathVariable("id") Long id) {
        TSysInterUrl sysInterUrl = sysInterUrlService.selectTSysInterUrlById(id);
    ...
    public AjaxResult remove(String ids) {
        int b = sysInterUrlService.deleteTSysInterUrlByIds(ids);
        if (b > 0) {
            return success();
        } else {
            return error();
    ...
    public String edit(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("SysInterUrl", sysInterUrlService.selectTSysInterUrlById(id));
    ...
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(TSysInterUrl record) {
        return toAjax(sysInterUrlService.updateTSysInterUrl(record));
    }
    ...
    public TSysInterUrl edit(@PathVariable("id") Long id) {
        return sysInterUrlService.selectTSysInterUrlById(id);
    }
