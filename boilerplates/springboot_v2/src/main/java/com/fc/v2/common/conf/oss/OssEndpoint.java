...
public class OssEndpoint {
    ...
    public AjaxResult createObject(@RequestBody MultipartFile object, @PathVariable String bucketName) throws Exception {
        ...
        if (putObjectResult != null) {
            ...
            TSysFile sysFile = null;
            if (tsysUser != null) {
                sysFile = new TSysFile(fileSuffixName, bucketName, object.getSize(), object.getContentType());
            } else {
                sysFile = new TSysFile(fileSuffixName, bucketName, object.getSize(), object.getContentType());
            }
            int i = sysFileService.insertTSysFile(sysFile);
            if (i > 0) {
    ...
    public AjaxResult createObject(@PathVariable("id") Long id, @RequestBody MultipartFile object, @PathVariable("bucketName") String bucketName) throws Exception {
        TSysFile oldSysFile = sysFileService.selectTSysFileById(id);
        if (oldSysFile != null) {
            String fileName = object.getOriginalFilename();
            String suffixName = fileName.substring(fileName.lastIndexOf(".")).toLowerCase();
            String uuid = SnowflakeIdWorker.getUUID();
            ...
            if (putObjectResult != null) {
                ...
                int i = sysFileService.updateTSysFile(oldSysFile);
                if (i > 0) {
                    return AjaxResult.successData(200, template.getObjectInfo(bucketName, fileSuffixName));
                }
            }
    ...
    public Object createObjectLayedit(@RequestBody MultipartFile file, @PathVariable String bucketName) throws Exception {
        ...
        if (putObjectResult != null) {
            ...
            TSysFile sysFile = null;
            if (tsysUser != null) {
                sysFile = new TSysFile(fileSuffixName, bucketName, file.getSize(), file.getContentType());
            } else {
                sysFile = new TSysFile(fileSuffixName, bucketName, file.getSize(), file.getContentType());
            }
            int i = sysFileService.insertTSysFile(sysFile);
